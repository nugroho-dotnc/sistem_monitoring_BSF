...
export const FeedingPage: React.FC = () => {
  ...
  const fetchLogs = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await api.getFeedingLogs(activeDeviceId);
      setLogs(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Yakin ingin menghapus data ini?')) {
      try {
        await api.deleteFeedingLog(activeDeviceId!, id);
        fetchLogs();
      } catch (err) {
        console.error(err);
        ...
        <div className="flex-1 flex items-center justify-center bg-slate-50 p-6">
          <div className="max-w-md text-center">
            ...
            <p className="text-slate-500 mb-8">
              Tambahkan perangkat kandang pertama Anda untuk mulai mencatat riwayat pakan.
            </p>
            <button 
      ...
      <main className="flex-1 max-w-[1280px] w-full mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold text-slate-900">Riwayat Pakan</h1>
                <p className="text-slate-500 text-sm">Kelola data pemberian pakan maggot untuk {activeDevice?.location}</p>
              </div>
            </div>
            <button 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-sm uppercase tracking-wider">
                    ...
                    <th className="px-6 py-4 font-semibold">Jenis Pakan</th>
                    ...
                    logs.map((log) => (
                      <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          {new Date(log.fed_at).toLocaleString('id-ID', {
